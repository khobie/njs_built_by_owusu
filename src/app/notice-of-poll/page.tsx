...
const STATUS_OPTIONS = ['ISSUED', 'IMPORTED', 'VETTED', 'APPROVED', 'REJECTED'];
...
function approvalBadgeClass(status: string): string {
  if (status === 'APPROVED') return 'nop-badge nop-approved';
  if (status === 'REJECTED') return 'nop-badge nop-rejected';
  return 'nop-badge nop-muted';
}

function NoticeOfPollInner() {
  ...
  const load = useCallback(async () => {
    ...
    setErr('');
    try {
      const res = await fetch(`/api/notice-of-poll?${queryString}`, { cache: 'no-store' });
      if (!res.ok) {
  ...
  useEffect(() => {
    void fetch('/api/electoral-areas', { cache: 'no-store' })
      .then((r) => (r.ok ? r.json() : []))
  ...
  const disqualifyAbsent = async () => {
    const confirmed = window.confirm(
      'Disqualify ALL applicants who have not been approved or rejected?\n\n' +
        'They will be marked REJECTED with the reason "Did not appear for vetting." ' +
        'Run this only after vetting is complete. Contest status will be recalculated.'
    );
    if (!confirmed) return;
    setBusy(true);
    setToast('');
    try {
      const res = await fetch('/api/notice-of-poll/disqualify-absent', { method: 'POST' });
      const j = await res.json().catch(() => ({}));
      if (!res.ok) {
  ...
  const exportBase = `/api/notice-of-poll/export?${queryString}`;
  ...
  return (
    <AppShell activeHref="/notice-of-poll">
      <div className="container nop-page">
        <header className="nop-header">
          <div>
            <h1>Notice of Poll</h1>
            <p>
              Official vetting outcomes, contest status, and election eligibility per electoral area
              and position. Contest status counts <strong>approved applicants only</strong>.
            </p>
          </div>
        ...
        <div className="nop-print-title only-print">
          <h2>NOTICE OF POLL — New Juaben South</h2>
              ...
              {loading && !data ? (
                ...
                data.rows.map((r: NoticeOfPollRow) => (
                  <tr key={r.id}>
                    ...
                    <td>{r.positionCanonical ?? r.position}</td>
                    <td>{r.applicantName}</td>
                    <td>{r.vettingStatus === 'VERIFIED' ? 'Vetted' : 'Not vetted'}</td>
                    <td>
                      <span className={approvalBadgeClass(r.approvalStatus)}>{r.approvalStatus}</span>
                    </td>
                    <td>
...
export default function NoticeOfPollPage() {
  return (
    <Suspense
      fallback={
        <AppShell activeHref="/notice-of-poll">
